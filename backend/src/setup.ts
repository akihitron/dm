    ...
    return (_context_ = await new Promise(async (resolve) => {
        ...
        try {
...

            {
                ...
                const CommonLimiter = USE_LIMITER
                    ? rateLimit({
                ...
                const SensitiveLimiter = USE_LIMITER
                    ? rateLimit({
            ...
            context.close = async function () {
                try {
                    await context.session_store?.destroy?.();
                } catch (e: any) {
                    console.error("\x1b[31m", e, "\x1b[0m");
                }
                try {
                    await context.session_store?.close?.();
                } catch (e: any) {
                    console.error("\x1b[31m", e, "\x1b[0m");
                }
                try {
