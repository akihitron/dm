...
import U from '../util';
import { DataGrid, GridColDef, GridRowParams, GridToolbarQuickFilter, GridValueGetterParams } from '@mui/x-data-grid';
import { useEffect, useState } from 'react';
import { Button, FormControl, InputLabel, Select, MenuItem, Alert, Card, CardContent, CardActions, TextField, FormControlLabel, Checkbox, Slider, CircularProgress, Tabs, Tab, Paper, CardHeader, Grid } from '@mui/material';
...
const TEMPLATE_NAME = [
	...
	'rabbit', 'cow', 'horse',
	'wolf', 'hippopotamus', 'kangaroo',
	'fox', 'giraffe', 'bear',
	'koala', 'bat', 'gorilla',
	'rhinoceros', 'monkey', 'deer',
	'zebra', 'jaguar', 'polar_bear',
	'skunk', 'elephant', 'raccoon_dog',
	'animal', 'reindeer', 'rat',
	'tiger', 'cat', 'mouse',
	'buffalo', 'hamster', 'panda',
	'sheep', 'leopard', 'pig',
	'mole', 'goat', 'lion',
	'camel', 'squirrel', 'donkey'
];
...
export default function InstancesGrid(prop: any) {
	...
	const [restriction_mode, setRestrictionMode] = useState("host");
	...
	useEffect(() => {
		...
		U.get('api/v1/port_map/list').then((ret: any) => {
			if (ret.data) {
		...
		U.get('api/v1/instance/list').then((ret: any) => {
			if (ret.data) {
				const data = ret.data.map((u: any) => Object.assign(u, {
		...
		<Box sx={{ width: '100%', marginBottom: 10 }}>
			...
			<Box sx={{ width: '100%', marginTop: 2, textAlign: processing_flag ? "center" : "right" }}>
				{processing_flag ? <CircularProgress /> : <>
					...
					}} color="primary" disabled={selectedRows.length == 0}>Start</Button>
					<Button sx={{ width: "20%", marginLeft: 1 }} variant="contained" onClick={() => {
						setButtonName("stop");
						setConfirmDialogVisibility(true);
					}} color="primary" disabled={selectedRows.length == 0}>Stop</Button>
					<Button sx={{ width: "20%", marginLeft: 1 }} variant="contained" onClick={() => {
						setConfirmDialogVisibility(true);
						setButtonName("restart");
					}} color="primary" disabled={selectedRows.length == 0}>Restart</Button>
					<Button sx={{ width: "20%", marginLeft: 1 }} variant="contained" onClick={() => {
						setButtonName("terminate");
						setConfirmDialogVisibility(true);
					}} color="primary" disabled={selectedRows.length == 0}>Terminate</Button>
			...
			{tab == 0 ? <>
				<Box sx={{ boxShadow: "0px 0px 2px black" }}>
					<Card>
						<CardContent>
							{error_message.length > 0 ? <Alert sx={{ marginBottom: 1, marginTop: 1 }} severity="error">{error_message}</Alert> : <></>}

							{processing_flag ? <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", width: "100%", height: "100%" }}>
								<CircularProgress />
							</Box> : <>

								<Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
									<Grid item xs={6}>
										<Accordion sx={{ backgroundColor: "#88888802" }} expanded={ac_ins_open} onChange={() => { setAcInsOpen(!ac_ins_open); }}>
											<AccordionSummary
												expandIcon={<ExpandMoreIcon />}
												aria-controls="panel1a-content"
												id="panel1a-header"
												sx={{ backgroundColor: "#88888808" }}
											>
												<Typography>Instance</Typography>
											</AccordionSummary>
											<AccordionDetails>
												{node_list && node_list.length == 0 ? <Alert sx={{ marginBottom: 1 }} severity="warning">{"No node. Please register a node."}</Alert> : <>
												...
												<TextField label="Instance name" variant="outlined" sx={{ marginBottom: 2, marginTop: 2, width: "100%" }}
													error={!!error_message_of_instance_name}
													helperText={error_message_of_instance_name}
													value={instance_name} onChange={(event: any) => {
									...
									<Grid item xs={6}>
										<Accordion sx={{ backgroundColor: "#88888802" }} expanded={ac_img_open} onChange={() => { setAcImgOpen(!ac_img_open); }}>
											<AccordionSummary
												expandIcon={<ExpandMoreIcon />}
											...
											<AccordionDetails>
												<FormControl fullWidth sx={{ marginTop: 2 }}>
													...
													>
														{image_list.filter(img => selected_node_for_create_instance && img.node_id == selected_node_for_create_instance.id).map((u: any, index: number) => <MenuItem key={index} value={u.id}>{u.name}  <Typography sx={{ color: "gray", marginLeft: 1 }}>[ {U.human_file_size_M(u.size)} ]</Typography></MenuItem>)}
													</Select>
												</FormControl>
											</AccordionDetails>
										</Accordion>
									...
									<Grid item xs={6}>
										...
										<Accordion sx={{ backgroundColor: "#88888802" }} expanded={ac_ssh_open} onChange={() => { setAcSSHOpen(!ac_ssh_open); }}>
											<AccordionSummary
												expandIcon={<ExpandMoreIcon />}
												...
												sx={{ backgroundColor: "#88888808" }}
											>
												<FormControlLabel control={<Checkbox checked={use_ssh} onClick={(event: any) => { event.stopPropagation(); toggleSSH(!use_ssh); }} />} label="SSH" />
											</AccordionSummary>

											<AccordionDetails>
												{ssh_list && ssh_list.length == 0 ? <Alert sx={{ marginBottom: 1 }} severity="warning">{"No SSH key. Please register a your public key first."}</Alert> : <>
													<FormControl fullWidth sx={{ marginTop: 2, width: "100%" }}>
														...
														>
															{ssh_list.map((u: any, index: number) => <MenuItem key={index} value={u.id}>{u.name}</MenuItem>)}
														</Select>
													</FormControl>
												</>}
									...
									<Grid item xs={6}>
										<Accordion sx={{ backgroundColor: "#88888802" }} expanded={ac_net_open} onChange={() => { setAcNetOpen(!ac_net_open); }}>
											<AccordionSummary
												expandIcon={<ExpandMoreIcon />}
											...
											<AccordionDetails>
												...
												<Box sx={{ padding: 2 }}>
													...
													<FormControlLabel control={<Checkbox checked={enable_cpu_limit} onClick={(event: any) => { event.stopPropagation(); setEnableCPULimit(!enable_cpu_limit); }} />} label="Enable CPU Limit" />
													<br />

													{enable_cpu_limit ? <><span>CPU Slicing ( {selected_node_for_create_instance?.cpu ?? 1} Cores ) : {selected_node_for_create_instance?.cpu_info ?? ""}</span><Slider
														aria-label="CPU"
													...
													<FormControlLabel control={<Checkbox checked={enable_memory_limit} onClick={(event: any) => { event.stopPropagation(); setEnableMemoryLimit(!enable_memory_limit); }} />} label="Enable Memory Limit" />
													<br />

													{enable_memory_limit ? <><span>Memory Cap ( {m_memory} MB )</span><Slider
														aria-label="Memory"
													...
													<FormControlLabel control={<Checkbox checked={enable_storage_limit} onClick={(event: any) => { event.stopPropagation(); setEnableStorageLimit(!enable_storage_limit); }} />} label="Enable Storage Limit" />
													<br />
													{enable_storage_limit ? <span><span>Storage Cap ( {g_storage} GB )</span><Slider
														aria-label="Storage"
													...
													/></span> : ""}
													{enable_storage_limit && selected_node_for_create_instance && selected_node_for_create_instance.manipulator_driver == "docker" ? <Alert sx={{ marginBottom: 1, marginTop: 1 }} severity="error">The docker does not support storage limitation for instance.</Alert> : <></>}
								...
								<Button sx={{ width: "100%", marginTop: 4 }} variant="contained" onClick={() => {
								...
								}} color="primary" disabled={disable_submit || use_ssh == true && ssh_list.length == 0}>
					...
					<Card>
						<CardContent>
							...
							<Button variant="outlined"
								...
								}}>{copied_message ? "Copied!" : "Copy to clipboard"}</Button>
						</CardContent>
					</Card>
				</Box>
			</> : ""}
					...
					(async () => {
						...
						for (const row of selectedRows) {
							if (buttonName == "terminate") {
								await U.post('api/v1/instance/delete', { instance_id: row }).then((ret: any) => {
									if (ret.error) {
								...
								await U.post('api/v1/instance/start', { instance_id: row }).then((ret: any) => {
									if (ret.error) {
										setOperationError(ret.error);
										console.error(ret.error);
									}
								...
								await U.post('api/v1/instance/stop', { instance_id: row }).then((ret: any) => {
									if (ret.error) {
										setOperationError(ret.error);
										console.error(ret.error);
									}
