...
export default function SSHKeysGrid(prop: any) {
    ...
    const handleSubmit = () => {
        if (TinySSHPublicKeyChecker(public_key) && name.length > 0) {
            ...
            setErrorLocation("");
            setSomethingError("");
            U.post("api/v1/ssh/register", { name: name.trim(), key: public_key.trim() })
                .then((ret: any) => {
                    if (ret.data) {
    ...
    return (
        <Box sx={{ height: 250, width: "100%", marginBottom: 10 }}>
            {error_location == "top" && something_error.length > 0 ? (
            ...
            <Box>
                ...
                <TextField label="Name" variant="outlined" fullWidth value={name} onChange={handleText1Change} margin="normal" />
                <TextareaAutosize style={{ width: "100%" }} value={public_key} onChange={handleText2Change} aria-label="minimum height" minRows={5} placeholder="Public Key" />
                ...
                onSubmit={(flag: boolean) => {
                    if (flag) {
                        ...
                        for (const id of selectedRows) {
                            U.post("api/v1/ssh/delete", { id: id })
                                .then((ret: any) => {
                                    if (ret.error) {
                                        setSomethingError(ret.error);
