...
import { Button, FormControl, InputLabel, Select, MenuItem, Alert, Card, CardContent, TextField, CircularProgress, CardHeader } from "@mui/material";
...
export default function ImagesGrid(prop: any) {
    ...
    useEffect(() => {
        U.get("api/v1/image/list")
            .then((ret: any) => {
                ...
                console.log(ret);
                if (ret.data) {
                    const _image_list = ret.data.map((u: any) =>
                        Object.assign(u, {
    ...
    return (
        <Box sx={{ width: "100%", marginBottom: 10 }}>
            ...
            {processing ? (
                <Box sx={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", marginTop: 1, marginBottom: 1 }}>
                    <CircularProgress size="1.5rem" />
                </Box>
            ) : (
                <>
                    <Button
                        sx={{ width: "49%", marginRight: "1%", marginTop: 2 }}
                        variant="contained"
                        onClick={async () => {
                            ...
                            for (const row of selected) {
                                ...
                                await U.post("api/v1/image/publish", body)
                                    .then((ret: any) => {
                                        if (ret.error) {
                                            if (ret.error_code == "x2TpbFQruG") window.location.reload();
                    ...
                    {!fetching ? (
                        <CardContent>
                            {node_list && node_list.length == 0 ? (
                                <Alert sx={{ marginBottom: 1, marginTop: 1 }} severity="warning">
                            ...
                            <TextField
                                label="Remote image url or alias"
                                variant="outlined"
                            ...
                            {fetch_success.length > 0 ? (
                                <Alert sx={{ marginBottom: 1, marginTop: 1 }} severity="success">
                                    {fetch_success}
                                </Alert>
                            ) : (
                                ""
                            ...
                            <Button
                                ...
                                color="primary"
                                disabled={disable_submit_to_fetch_image}
                            >
                                Fetch Image
                            </Button>
                        </CardContent>
                ...
                onSubmit={async (flag: boolean) => {
                    if (flag) {
                        ...
                        for (const row of selectedRows) {
                            await U.post("api/v1/image/delete", { image_id: row })
                                .then((ret: any) => {
                                    if (ret.error) {
                                        if (ret.error_code == "x2TpbFQruG") window.location.reload();
                                        setModifyError(ret.error ?? "");
