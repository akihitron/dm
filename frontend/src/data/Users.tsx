...
import { Alert, Button, Card, CardContent, TextField, TextareaAutosize } from '@mui/material';
...
export default function UsersGrid(prop: any) {
	...
	useEffect(() => {
		(async () => {
			if (is_logged_in) {
				U.get('api/v1/user/list').then((ret: any) => {
			...
			<Card>
				...
				if (flag) {
					...
					for (const id of selectedRows) {
						U.post('api/v1/user/delete', { user_id: id }).then((ret: any) => {
							if (ret.error) {
								setSomethingError(ret.error);
